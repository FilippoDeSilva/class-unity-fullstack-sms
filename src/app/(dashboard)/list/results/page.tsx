...
const renderRow = (item: ResultList) => (
  ...
  const data = dataRes.map((item) => {
    // Find the assessment based on courseName and studentId
    const assessment = 
      item.courseName === item.courseName && item.studentId === item.studentId
        ? item
        : null;
  
    if (!assessment) return null; // If no assessment found, skip this item
  
    // Prepare the returned data object
    return {
      id: item.id,
      title: item.courseName, // Course name as the title
      studentName: item.student.name, // Student name
      studentSurname: item.student.surname, // Student surname
      teacherName: item.teacherName, // Teacher's name
      teacherSurname: item.teacherSurname, // Teacher's surname
      score: item.score, // Final score
      className: item.className, // Class name
      startTime: item.startTime, // Start time for the course
      courseName: item.courseName, // Course name
      midexam: item.midexam, // Mid-exam score
      assignmentRe: item.assignmentRe, // Assignment score
    };
  });

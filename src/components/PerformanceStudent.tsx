...
const Performance = () => {
  ...
  // Fetch results using studentId from Clerk
  useEffect(() => {
    if (isLoaded && user) {
      const studentId = user.id; // Get the studentId from the logged-in user

      const fetchResults = async () => {
        try {
          const response = await axios.get(`/api/student/${studentId}`); // Fetch data with studentId
  ...
  }, [isLoaded, user]); // Refetch when the user data is loaded
